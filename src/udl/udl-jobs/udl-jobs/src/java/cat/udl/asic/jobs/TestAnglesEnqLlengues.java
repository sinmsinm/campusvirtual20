...
public class TestAnglesEnqLlengues implements Job {

	//Inicialitzem el log
	static Logger M_log = Logger.getLogger(TestAnglesEnqLlengues.class);
	...
	static String	SQL_TOTS_VISTA = "SELECT LOGIN FROM ENQUESTA_LLENG WHERE P7='ENG' AND P4_DEU='no' AND P4_ENG='no' AND P4_FRA='no' AND P4_ITA='no' AND LOGIN NOT IN (SELECT LOGIN FROM TRACTATS_PROVA_NIVELL)";
	static String	SQL_MARCA_TRACTAT = "INSERT INTO TRACTATS_PROVA_NIVELL(LOGIN,DATA_TRACTAT) VALUES (?,SYSDATE)";

	static String	SQL_TOTS_TERCERCES_LLENGUES = "SELECT LOGIN FROM ENQUESTA_LLENG WHERE P7!='ENG' AND P4_DEU='no' AND P4_ENG='no' AND P4_FRA='no' AND P4_ITA='no' AND LOGIN NOT IN (SELECT LOGIN FROM TRACTATS_TERCERES_LLENGUES)";
	static String	SQL_MARCA_TRACTAT_TERCERES_LLENGUES = "INSERT INTO TRACTATS_TERCERES_LLENGUES(LOGIN,DATA_TRACTAT) VALUES (?,SYSDATE)";
	...
	public void init() {
        ...
        M_log.debug("Inicia JOB Prova Nivell Primer");
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		M_log.info("EnqueLleng: Execució JOB enquesta primer");
		...
		try {
					...
					if (login != null) {
						...
						if (user != null) {
							...
							if (preferences != null) {
								...
								props.addProperty("udlTestAngles", "1");	
								preferencesService.commit(preferences);

								M_log.info ("EnqLleng: propietat afegida a " + login);
								
								// Desem el registre com a tractat
								try {
									stmtins = connection.prepareStatement(SQL_MARCA_TRACTAT);
									stmtins.setString(1, login);
									stmtins.executeUpdate();
									stmtins.close();
									M_log.info ("EnqLleng: Marcat com a tractat " + login);
								} catch (Exception sqlInsException) {
									M_log.error("EnqLleng: Error desant el registre com marcat " + login + sqlInsException);
									sqlInsException.printStackTrace ();
								}
							}
			...
			stmt = connection.prepareStatement(SQL_TOTS_TERCERCES_LLENGUES);
					...
					if (login != null) {
						...
						if (user != null) {
							...
							if (preferences != null) {
								// Afegim la propietat
								ResourcePropertiesEdit props = preferences.getPropertiesEdit();
								props.addProperty("udlTestTerceresLlengues", "1");	
								preferencesService.commit(preferences);
								...
								// Desem el registre com a tractat
								try {
									stmtins = connection.prepareStatement(SQL_MARCA_TRACTAT_TERCERES_LLENGUES);
									stmtins.setString(1, login);
									stmtins.executeUpdate();
									stmtins.close();
									M_log.info ("EnqLleng: Marcat com a tractat " + login);
