...
public class EnquestaLlengues implements Job {

	//Inicialitzem el log
	static Logger M_log = Logger.getLogger(EnquestaLlengues.class);
	...
	static String	SQL_TOTS_VISTA = "SELECT LOGIN FROM VUDL_LOGIN_MAT_NOUS@MIGRA WHERE LOGIN NOT IN (SELECT LOGIN FROM TRACTATS)";
	static String	SQL_MARCA_TRACTAT = "INSERT INTO TRACTATS (LOGIN,DATA_TRACTAT) VALUES (?,SYSDATE)";
	...
	public void init() {
        ...
        M_log.debug("Inicia JOB EnquestaPrimer");
			...
			while (rst.next()) {
				...
				try {
					...
					if (login != null) {
						...
						M_log.info ("EnqLleng: Processant usuari " + login);
						...
						if (user != null) {
							...
							if (preferences != null) {
								...
								props.addProperty("udlFirstYearTutorial", "1");	
